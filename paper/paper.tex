...
%Aims
In this work we address the issue that the \LS estimator evaluates the \cf in bins of separation between objects, which results in a loss of information and a trade-off between bias and variance. \KSF{there are other aims but they are hard to describe, as they are not ``issues'' with the LS estimator but other capabilities of ours - how to phrase?}
%Methods
...
\centering
    ...
    \caption{A comparison between \est with a cubic spline basis function (red dotted) and the standard tophat estimator (blue solid). The shaded region is the 1$\sigma$ variation in the 100 mock catalogs. The cubic spline estimator has lower bias and lower variance with fewer bins. \KSF{move rmse from figure into text only? but then not necessarily clear its less biased} \KSF{does tophat need to be a different linestyle?}}
\end{figure}
...
\KSF{BUT my estimator is having a hard time recovering the truth anyway! Because of variance in the realizations. It converges to the same, but wrong, value of alpha, and diff ones for diff realizations. Need to average over many?}
...
These basis functions are significantly different than the tophat or B-spline bases previously explored as they are not localized.
This means that data at all scales could contribute to all basis functions.
It is then critical to ensure that the final parameter estimate does not reply on the range of scales chosen.
...
Another potential use case is the luminosity and color dependence of galaxy clustering, which can be used to probe galaxy formation \citep{Zehavi20}.
